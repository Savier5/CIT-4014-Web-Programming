...
// Savier Osman
// 12/05/2022
// Replace [[:digit:]] with \d in phone() and zip()
...
class Validate {
    ...
    public function phone($name, $value, $required = false) {
        ...
        // Removed [[:digit:]] and replaced it with \d
        $pattern = '/^\d{3}-\d{3}-\d{4}$/';
    ...
    public function birthdate($name, $value, $required = true) {
        ...
        // Call the text method and exit if it fields an error
        $this->text($name, $value, $required);
        if ($field->hasError()) { return; }

        // Call the pattern method to validate a birthdate
    ...
    public function email($name, $value, $required = true) {
        ...
        //Use the filter_var method to validate the email address
        ...
        /* Removed manual email validation routine
    ...
    public function password($name, $password, $required = true) {
        //Modified function to required minimum of 8 characters and only upper digit.
        $field = $this->fields->getField($name);
        ...
        $charClasses[] = '[:digit:]';
        $charClasses[] = '[:upper:]';
        // $charClasses[] = '[:lower:]'; Removed for exercise
        // $charClasses[] = '_-'; Removed for exercise

        $pw = '/^';
        //$valid = '['; Removed for exercise
        foreach($charClasses as $charClass) {
            $pw .= '(?=.*[' . $charClass . '])';
            //$valid .= $charClass; Removed for exercise
        }
        $valid = '.{8,}'; // Changed to any 8 characters
        $pw .= $valid . '$/';
